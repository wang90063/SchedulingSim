...
class CliSmokeTests(unittest.TestCase):
    ...
    def test_systematic_simulation_analysis_runner_writes_manifest_and_tables(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("## Insight Summary", summary_report)
            self.assertIn("## Background Cost and Resource Analysis", summary_report)
            self.assertIn("Scene points evaluated: `1`", summary_report)
            self.assertIn("Positive-gain scene points:", summary_report)
            self.assertIn("Zero-gain scene points:", summary_report)
            self.assertIn("Negative-gain scene points:", summary_report)
